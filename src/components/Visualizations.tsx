...
// Lazy load ScatterView supaya tab lain tidak berat
const ScatterView = dynamic(() => import("./viz/ScatterView"), {
    ssr: false,
    loading: () => <div className="text-sm text-gray-600">Loading scatter…</div>,
});
...
export default function Visualizations({ rows, schema }: Props) {
    const [active, setActive] = useState<(typeof TABS)[number]["key"]>("trend");
    ...
    return (
        <section className="mt-10 rounded-2xl border border-gray-200 bg-white p-6 md:p-8">
            <div className="flex items-center gap-3">
                ...
                <h2 className="text-2xl font-semibold text-gray-800">Visualizations</h2>
            </div>
            ...
            <div className="mt-6">
                {active === "trend" && <TrendView rows={rows} schema={schema} />}
                {active === "hist" && <HistogramView rows={rows} schema={schema} />}
                {active === "scatter" && <ScatterView rows={rows} schema={schema} />}
            </div>
        </section>
    );
}
